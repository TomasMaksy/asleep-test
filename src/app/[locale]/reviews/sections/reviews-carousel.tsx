...
export type ReviewsCarouselItem = {
  title: string;
  body: string;
  date: string;
};

export type ReviewsCarouselCopy = {
  items: ReviewsCarouselItem[];
  prev: string;
  next: string;
};
...
function ReviewColumn({ item }: { item: ReviewsCarouselItem }) {
  return (
...
/** Motion carousel only — copy comes from a server parent (see layers pattern). */
export function ReviewsCarousel({ copy }: { copy: ReviewsCarouselCopy }) {
  const { items, prev, next } = copy;
  ...
  const measure = useCallback(() => {
    ...
    setMaxIndex(nextMax);
    setIndex((current) => {
      const nextIndex = Math.min(current, nextMax);
      x.set(nextStep ? -Math.min(nextIndex * nextStep, nextMaxScroll) : 0);
      return nextIndex;
    });
  }, [x]);
