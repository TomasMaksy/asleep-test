...
const ProductStickyBuyBar = dynamic(
  () =>
    import("@/app/[locale]/products/original/product-sticky-buy-bar").then(
      (mod) => ({ default: mod.ProductStickyBuyBar }),
    ),
  { ssr: false },
);

export type ProductBuyControlsCopy = {
  sizeLabel: string;
  heightLabel: string;
  saveBadge: string;
  deliveryNote: string;
  ...
  configurator: string;
  productName: string;
  stickyProductName: string;
};

type ProductBuyControlsProps = {
  copy: ProductBuyControlsCopy;
};

export function ProductBuyControls({ copy }: ProductBuyControlsProps) {
    ...
    <>
      ...
      <div className="mb-4 rounded-2xl border border-grey bg-surface p-4">
        ...
        <p className="mt-2 flex items-center gap-2 text-brand-dark text-sm">
          <InbankMark />
          <span>{copy.installment.replace("{price}", installmentPrice)}</span>
      ...
      <ProductStickyBuyBar
        ...
        productName={copy.stickyProductName}
        saveBadge={copy.saveBadge}
